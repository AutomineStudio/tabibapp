...
async function loadCSV() {
  ...
  const nameIdx = pickHeaderIndex(headers, [["medicine_name"], ["name"], ["medicament"], ["drug"], ["product"], ["nom"]]);
  const classIdx = pickHeaderIndex(headers, [["therapeutic_class"], ["class"], ["classe"], ["therapeutique"], ["category"], ["famille"]]);
  // Price comes from the price_of_the_medicine_in_dirhams column in our dataset
  const priceIdx = pickHeaderIndex(
    headers,
    [["price_of_the_medicine_in_dirhams"], ["ppv"], ["price_mad"], ["price"], ["prix"], ["retail"], ["mad"], ["dh"]]
  );
  ...
  const toNumber = (val) => {
    const s = String(val ?? '')
      ...
      .replace(/ppv\s*/i, '') // remove "ppv" prefix
      .replace(/\s*dhs?/i, '') // remove "dhs" suffix
      .trim();
