...
export async function searchMedicines({
  ...
  const results = medicines.filter(med => {
    ...
    if (matches && (patientAge !== null || patientGender !== null)) {
      ...
      // Pregnancy and breastfeeding filtering
      if (isPregnant || isBreastfeeding) {
        ...
        if (isPregnant) {
          const pregnancyKeywords = ['grossesse', 'enceinte', 'femme enceinte', 'gestation'];

          // Check if medicine mentions pregnancy AND contraindication
          const mentionsPregnancy = pregnancyKeywords.some(keyword =>
            indicationsLower.includes(keyword)
          );

          if (mentionsPregnancy) {
            const hasContraindication = contreindicationsKeywords.some(keyword =>
              indicationsLower.includes(keyword)
            );

            // If there's a contraindication related to pregnancy, filter out
            if (hasContraindication) {
              // Check if the contraindication is specifically about pregnancy
              const contraindicationText = indicationsLower;
              const contraindicationNearPregnancy = pregnancyKeywords.some(pregKeyword => {
                const pregIndex = contraindicationText.indexOf(pregKeyword);
                if (pregIndex === -1) return false;

                // Check if any contraindication keyword is within 100 characters
                return contreindicationsKeywords.some(contraKeyword => {
                  const contraIndex = contraindicationText.indexOf(contraKeyword);
                  return contraIndex !== -1 && Math.abs(contraIndex - pregIndex) < 100;
                });
              });

              if (contraindicationNearPregnancy) {
                return false; // Skip this medicine for pregnant women
              }
            }
          }
        }

        // Check for breastfeeding contraindications
        if (isBreastfeeding) {
          const breastfeedingKeywords = ['allaitement', 'lactation', 'allaitante', 'nourrice'];

          const mentionsBreastfeeding = breastfeedingKeywords.some(keyword =>
            indicationsLower.includes(keyword)
          );
          ...
          if (mentionsBreastfeeding) {
            ...
            if (hasContraindication) {
              // Check if the contraindication is specifically about breastfeeding
              const contraindicationText = indicationsLower;
              const contraindicationNearBreastfeeding = breastfeedingKeywords.some(bfKeyword => {
                const bfIndex = contraindicationText.indexOf(bfKeyword);
                if (bfIndex === -1) return false;

                return contreindicationsKeywords.some(contraKeyword => {
                  const contraIndex = contraindicationText.indexOf(contraKeyword);
                  return contraIndex !== -1 && Math.abs(contraIndex - bfIndex) < 100;
                });
              });

              if (contraindicationNearBreastfeeding) {
                return false; // Skip this medicine for breastfeeding women
              }
            }
          }
        }
